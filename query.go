...
//from github.com/chihaya/chihaya/http/query
...
func NewSliced(query string) (*SliceQuery, error) {
	var (
		keyStart, keyEnd int
		valStart, valEnd int
		firstInfohash string
	...
	for i, length := 0, len(query); i < length; i++ {
		...
		if separator || last {
			...
			q.Params[strings.ToLower(keyStr)] = append(q.Params[strings.ToLower(keyStr)],valStr)
